  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-[28px] font-bold tracking-tight text-[var(--color-fg)]">Despesas</h1>
          <p className="text-sm text-[var(--color-fg-muted)] capitalize">
            Total efetivado em {monthLabel}: <span className="font-semibold tabular-nums" style={{ color: 'var(--color-danger)' }}>{formatCurrency(total)}</span>
          </p>
        </div>
        <input
          type="month"
          ...
          <div className="flex flex-col">
            {transactions.map(t => (
              <div key={t.id} className="tx-row">
                <div
                  className="tx-icon"
                  style={{ backgroundColor: t.category_color || 'var(--color-danger)' }}
                >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="tx-amount tabular-nums" style={{ color: 'var(--color-danger)' }}>{formatCurrency(Number(t.amount))}</span>
                  {t.status === 'pending' && (
