      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 capitalize">
            Total efetivado em {monthLabel}: <span className="font-semibold text-emerald-600">{formatCurrency(total)}</span>
          </p>
        </div>
        <input
          type="month"
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        {transactions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-gray-400">Nenhuma receita neste período</p>
          </div>
        ) : (
              ...
              <div key={t.id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ backgroundColor: t.category_color || '#10B981' }}
                  >
                    {t.category_name?.[0] ?? '?'}
                  </div>
                  <div>
